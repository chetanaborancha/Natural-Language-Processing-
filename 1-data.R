...
df5 <- arrange(df4, desc(Grad.Rate))
select(df5, X, (Grad.Rate))
...
df6 <- group_by(df, Private)
df6 <- summarise(df6, max=max(Outstate),min=min(Outstate))
df6
...
##  max and min tuition ("Outstate") grouped by private/public in dataset df6
df6 <- group_by(df, Private)
df6 <- summarise(df6, max=max(Outstate),min=min(Outstate))
df6

##  max and min tuition ("Outstate") grouped by private/public in dataset college
df7 <- group_by(college, Private)
df7
